...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for l in range(recur_layers):
        inp = input_data if l == 0 else bn_rnn
        simple_rnn = GRU(units, activation=activation,
                     return_sequences=True, implementation=2, name='rnn'+str(l))(inp)
...
#                cnn_layers=1, recur_layers=1, output_dim=29, dropout=0.4, pool_size=2):
    ...
    for l in range(cnn_layers):
        inp = input_data if l == 0 else bn_cnn
        conv_1d = Conv1D(filters, kernel_size, 
                         strides=conv_stride, 
                         padding=conv_border_mode,
                         activation='relu',
                         name='conv1d')(input_data)
        pool_cnn = MaxPooling1D(pool_size)(conv_1d)
        bn_cnn = BatchNormalization(name='bn_conv_1d')(pool_cnn)
        
    cnn_out = Dropout(rate=dropout)(bn_cnn)
    
    activation='relu'
    for l in range(recur_layers):
        inp = cnn_out if l == 0 else bn_rnn
        simple_rnn = GRU(units, activation=activation,
                     return_sequences=True, implementation=2, name='rnn'+str(l))(inp)
        bn_rnn = BatchNormalization()(simple_rnn)

    dropout_rnn = Dropout(rate=dropout)(bn_rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    ...
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
    model.output_length = lambda x: cnn_output_length_with_pooling(
        x, kernel_size, conv_border_mode, conv_stride,pooling=pool_size)
    print(model.summary())
    return model

def cnn_output_length_with_pooling(input_length, filter_size, border_mode, stride,
